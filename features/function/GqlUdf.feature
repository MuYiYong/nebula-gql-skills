...
Feature: GQL User-defined function
  ...
  Scenario: GQL UDF test
    When executing query:
      """
      CREATE FUNCTION IF NOT EXISTS _pi() RETURNS float AS {3.1415926}
      """
    ...
    When executing query:
      """
      show GQL user defined function _pi
      """
    Then the result should be, in any order:
      | name  | signature                      | function_type |
      | "_pi" | "_pi() -> FLOAT { 3.1415926 }" | "[GQL UDF]"   |
    When executing query:
      """
    ...
    Then the result should be, in any order:
      | name      | signature                                          | function_type |
      | "avg_sum" | "avg_sum(b FLOAT) -> FLOAT { avg(b)+sum(abs(b)) }" | "[GQL UDF]"   |
    When executing query:
      """
