...
Feature: Dry Run nebula_exec
  ...
  Scenario: DML operations test
    When executing query:
      ...
      CALL nebula_exec("nebula://root:NebulaGraph01@${TCK_GRAPH_ADDRESS}", "CREATE GRAPH IF NOT EXISTS dry_run_ldbc_remote_test TYPED ldbc_type") FINISH
      """
    ...
    Then the result should not contain:
      ...
      | "dry_run_ldbc_remote_test" | "ldbc_type" | "/default_schema" | "root" | ""    |
    When executing query:
      """
      CALL nebula_exec("nebula://root:NebulaGraph01@${TCK_GRAPH_ADDRESS}", "CREATE GRAPH IF NOT EXISTS dry_run_ldbc_remote_test TYPED ldbc_type") FINISH
      """
    ...
    When executing query:
      """
      /*+ SET_VAR(dry_run=true) */
      CALL nebula_exec("nebula://root:NebulaGraph01@${TCK_GRAPH_ADDRESS}", "DROP GRAPH IF EXISTS dry_run_ldbc_remote_test")
      FINISH
      """
    Then the execution should be successful
    When executing query:
  ...
  Scenario: Negative case - Basic errors
    When executing query:
      """
      /*+ SET_VAR(dry_run=true) */
      CALL nebula_exec("invalid_connection_string", "USE dry_run_ldbc_remote_test")
      FINISH
      """
    ...
    When executing query:
      """
      /*+ SET_VAR(dry_run=true) */
      CALL nebula_exec("nebula://root:NebulaGraph01@255.255.255.255:26692", "USE dry_run_ldbc_remote_test") FINISH
      """
  ...
  Scenario: DML operations with TLS enabled
    ...
    When executing query:
      ...
      "CREATE GRAPH IF NOT EXISTS dry_run_ldbc_remote_tls_test TYPED ldbc_type") FINISH
      """
    Then the execution should be successful
    ...
    When executing query:
      ...
      /*+ SET_VAR(dry_run=true) */
      CALL nebula_exec("nebula://root:NebulaGraph01@${TCK_GRAPH_ADDRESS}?tls_enable=true&tls_ca=${TCK_GRAPH_TLS_CA_CERT}&tls_cert=${TCK_GRAPH_TLS_CERT}&tls_key=${TCK_GRAPH_TLS_KEY}&tls_peer_name=graph.server.vesoft.com", "
      USE dry_run_ldbc_remote_tls_test
      INSERT
    ...
    When executing query:
      """
      CALL nebula_exec("nebula://root:NebulaGraph01@${TCK_GRAPH_ADDRESS}?tls_enable=true&tls_ca=${TCK_GRAPH_TLS_CA_CERT}&tls_cert=${TCK_GRAPH_TLS_CERT}&tls_key=${TCK_GRAPH_TLS_KEY}", "DROP GRAPH IF EXISTS dry_run_ldbc_remote_tls_test")
      FINISH
      """
    Then the execution should be successful
