...
Feature: Dry Run Temporary graph
  ...
  Scenario: Mutable temporary graph indexes
    ...
    When executing query:
      """
      /*+ SET_VAR(dry_run=true) SET_VAR(optimizer_rules = "get_node_to_node_scan=off") */
    ...
    When executing query:
      """
      /*+ SET_VAR(dry_run=true) SET_VAR(optimizer_rules = "get_edge_to_edge_scan=off") */
      USE #dry_run_temp_index_graph
  ...
  Scenario: export and import sf01 data to csv/parquet/orc
    ...
    When executing graph query:
      ...
      = DATAFILE {PATH:"file://${TEST_DIR}/dataset/dry_run_temp_test_files/sf01_post_csv",  FORMAT:'csv'}
      FILE f_parquet {id int, content string, creationDate string}
      = DATAFILE {PATH:"file://${TEST_DIR}/dataset/dry_run_temp_test_files/sf01_post_parquet",  FORMAT:'parquet'}
      FILE f_orc  {id int, content string, creationDate string}
      = DATAFILE {PATH:"file://${TEST_DIR}/dataset/dry_run_temp_test_files/sf01_post_orc",  FORMAT:'orc'}
    ...
    When executing graph query:
      ...
      PATH:"file://${TEST_DIR}/dataset/dry_run_temp_test_files/sf01_post_csv",
      FORMAT:'csv',
      delimiter: ","
      },
      NODE (v@post_parquet{id:id, content:content, creationDate:creationDate} ) FROM DATAFILE {
      PATH:"file://${TEST_DIR}/dataset/dry_run_temp_test_files/sf01_post_parquet",
      FORMAT:'parquet'
      },
      NODE (v@post_orc{id:id, content:content, creationDate:creationDate} )  FROM DATAFILE {
      PATH:"file://${TEST_DIR}/dataset/dry_run_temp_test_files/sf01_post_orc",
      FORMAT:"orc"
      }
    ...
    And drop the graph "#dry_run_import_export_multiple_format_file_g"
    And drop the graph type "dry_run_import_export_multiple_format_file_gt"
    Then remove the temporary directory "${TEST_DIR}/dataset/dry_run_temp_test_files/sf01_post_csv"
    Then remove the temporary directory "${TEST_DIR}/dataset/dry_run_temp_test_files/sf01_post_parquet"
    Then remove the temporary directory "${TEST_DIR}/dataset/dry_run_temp_test_files/sf01_post_orc"
