...
Feature: Distributed Graph Recoding

  Scenario: multi-type CSV data
    When executing analytic query:
      """
      CREATE GRAPH TYPE IF NOT EXISTS multi_type_graph_type AS {
    ...
    When executing analytic query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #multi_type_users TYPED multi_type_graph_type PARTITION BY DEFAULT
      """
    Then the execution should be successful
    When executing analytic query:
      """
      /*+ SET_VAR(query_concurrency = 2) */
      USE #multi_type_users IMPORT INTO GRAPH
      {
    ...
    When executing analytic query:
      """
      USE #multi_type_users SHOW STATS
      """
    ...
    When executing analytic query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #multi_type_companies TYPED multi_type_graph_type PARTITION BY DEFAULT
      """
    Then the execution should be successful
    When executing analytic query:
      """
      /*+ SET_VAR(query_concurrency = 2) */
      USE #multi_type_companies IMPORT INTO GRAPH
      {
        NODE (v@company{id_str:f.company_id, industry:f.industry, employee_count:f.employee_count}) FROM DATAFILE f {
          PATH: "file://${TEST_DIR}/tests/dataset/external_source/multi_type/node/company.csv",
          FORMAT: "csv",
          delimiter: ","
        }
      } OPTIONS {PRIMARY_KEY_AS_NODE_ID:false}
    ...
    When executing analytic query:
      """
      USE #multi_type_companies SHOW STATS
      """
    Then the result should be, in any order:
      | entry_name   | element_type | total_num |
      | "Node Total" | "Node"       | 15        |
    ...
    When executing analytic query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #multi_type_products TYPED multi_type_graph_type PARTITION BY DEFAULT
      """
    Then the execution should be successful
    When executing analytic query:
      """
      /*+ SET_VAR(query_concurrency = 2) */
      USE #multi_type_products IMPORT INTO GRAPH
      {
    ...
    When executing analytic query:
      """
      USE #multi_type_products SHOW STATS
      """
    Then the result should be, in any order:
      | entry_name   | element_type | total_num |
      | "Node Total" | "Node"       | 20        |
    ...
    When executing analytic query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #multi_type_complete TYPED multi_type_graph_type PARTITION BY DEFAULT
      """
    Then the execution should be successful
    When executing analytic query:
      """
      /*+ SET_VAR(query_concurrency = 4) */
      USE #multi_type_complete IMPORT INTO GRAPH
      {
        NODE (v@user{id_str:user_id, name:name, age:age, email:email, score:score}) FROM DATAFILE {
          PATH: "file://${TEST_DIR}/dataset/external_source/multi_type/node/user.csv",
          FORMAT: "csv",
    ...
    When executing analytic query:
      """
      USE #multi_type_complete SHOW STATS
      """
    Then the result should be, in any order:
      | entry_name   | element_type | total_num |
      | "Node Total" | "Node"       | 75        |
    ...
    When executing analytic query:
      """
      USE #multi_type_complete {
        VALUE user_count SumAgg<INT> = 0
        VALUE product_count SumAgg<INT> = 0
        VALUE company_count SumAgg<INT> = 0
        VALUE order_count SumAgg<INT> = 0
        VALUE purchase_count SumAgg<INT> = 0
        VALUE work_count SumAgg<INT> = 0
        ...
        RETURN @user_count, @product_count, @company_count, @order_count, @purchase_count, @work_count
      }
      """
    Then the result should be, in any order:
      | @user_count | @product_count | @company_count | @order_count | @purchase_count | @work_count |
      | 20          | 20             | 15             | 20           | 25              | 15          |
    When executing analytic query:
      """
      USE #multi_type_complete {
    ...
    And drop the graph "#multi_type_users"
    And drop the graph "#multi_type_companies"
    And drop the graph "#multi_type_products"
    And drop the graph "#multi_type_complete"
    And drop the graph type "multi_type_graph_type"

  Scenario: invalid data test
    When executing analytic query:
      """
      CREATE GRAPH TYPE IF NOT EXISTS invalid_data_test AS {
        NODE TYPE nproduct (LABEL nproduct {id INT PRIMARY KEY, name STRING, category STRING, price DOUBLE, stock INT}),
        NODE TYPE order_node (LABEL order_node {id INT PRIMARY KEY, status STRING, total_amount DOUBLE, order_time INT}),
        EDGE TYPE contains (order_node)-[LABEL contains {quantity INT, unit_price DOUBLE, MULTIEDGE KEY()}]->(nproduct)
      }
    ...
    When executing analytic query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #dangling_contains TYPED invalid_data_test PARTITION BY DEFAULT
      """
    Then the execution should be successful
    When executing analytic query:
      """
    ...
    When executing analytic query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #dangling_contains_new TYPED invalid_data_test PARTITION BY DEFAULT
      """
    Then the execution should be successful
    When executing analytic query:
      """
      /*+ SET_VAR(query_concurrency = 4) */
      USE #dangling_contains_new IMPORT INTO GRAPH
      {
        NODE (v@nproduct{id:product_id, name:name, category:category, price:price, stock:stock}) FROM DATAFILE {
          PATH: "file://${TEST_DIR}/dataset/external_source/multi_type/node/product.csv",
          FORMAT: "csv",
    ...
    When executing analytic query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #null_pk_order TYPED invalid_data_test PARTITION BY DEFAULT
      """
    Then the execution should be successful
    When executing analytic query:
      """
      /*+ SET_VAR(query_concurrency = 4) */
      USE #null_pk_order IMPORT INTO GRAPH
      {
        NODE (v@nproduct{id:product_id, name:name, category:category, price:price, stock:stock}) FROM DATAFILE {
          PATH: "file://${TEST_DIR}/dataset/external_source/multi_type/node/product.csv",
          FORMAT: "csv",
    ...
    When executing analytic query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #null_pk_contains TYPED invalid_data_test PARTITION BY DEFAULT
      """
    Then the execution should be successful
    When executing analytic query:
      """
      /*+ SET_VAR(query_concurrency = 4) */
      USE #null_pk_contains IMPORT INTO GRAPH
      {
        NODE (v@nproduct{id:product_id, name:name, category:category, price:price, stock:stock}) FROM DATAFILE {
          PATH: "file://${TEST_DIR}/dataset/external_source/multi_type/node/product.csv",
          FORMAT: "csv",
    ...
    When executing analytic query:
      """
      USE #null_pk_contains SHOW STATS
      """
    Then the result should be, in any order:
      | entry_name   | element_type | total_num |
      | "Node Total" | "Node"       | 40        |
    ...
    When executing analytic query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #null_pk_contains_1 TYPED invalid_data_test PARTITION BY DEFAULT
      """
    Then the execution should be successful
    When executing analytic query:
      """
      /*+ SET_VAR(query_concurrency = 4) */
      USE #null_pk_contains_1 IMPORT INTO GRAPH
      {
        NODE (v@nproduct{id:product_id, name:name, category:category, price:price, stock:stock}) FROM DATAFILE {
          PATH: "file://${TEST_DIR}/dataset/external_source/multi_type/node/product.csv",
          FORMAT: "csv",
    ...
    When executing analytic query:
      """
      USE #null_pk_contains_1 SHOW STATS
      """
    Then the result should be, in any order:
      | entry_name   | element_type | total_num |
      | "Node Total" | "Node"       | 40        |
      | "Edge Total" | "Edge"       | 17        |
    And drop the graph "#dangling_contains_new"
    And drop the graph "#null_pk_order"
    And drop the graph "#null_pk_contains"
    And drop the graph "#null_pk_contains_1"
    And drop the graph type "invalid_data_test"

  Scenario: error cases
    When executing analytic query:
      """
      CREATE GRAPH TYPE IF NOT EXISTS order_type AS {
        NODE TYPE nproduct (LABEL nproduct {id INT PRIMARY KEY, name STRING, category STRING, price DOUBLE, stock INT}),
        NODE TYPE order_node (LABEL order_node {id INT PRIMARY KEY, status STRING, total_amount DOUBLE, order_time INT}),
        EDGE TYPE contains (order_node)-[LABEL contains {quantity INT, unit_price DOUBLE, MULTIEDGE KEY()}]->(nproduct)
      }
    ...
    When executing analytic query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #product_error TYPED order_type PARTITION BY DEFAULT
      """
    Then the execution should be successful
    When executing analytic query:
      """
      /*+ SET_VAR(query_concurrency = 2) */
      USE #product_error IMPORT INTO GRAPH
      {
        NODE (v@nproduct{id:id, name:name, category:category, price:price, stock:stock}) FROM DATAFILE {
          PATH: "file://${TEST_DIR}/dataset/external_source/multi_type/node/product.csv",
          FORMAT: "csv",
          delimiter: ","
        }
      ...
      """
    Then an Error should be raised: "[NP401]: Connector error: Failed to convert node/edge type: nproduct, error: Connector error: Column: 'id' not found in source schema"
    And drop the graph "#product_error"
    When executing analytic query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #contains_error TYPED order_type PARTITION BY DEFAULT
      """
    Then the execution should be successful
    When executing analytic query:
      """
      /*+ SET_VAR(query_concurrency = 4) */
      USE #contains_error IMPORT INTO GRAPH
      {
        NODE (v@nproduct{id:product_id, name:name, category:category, price:price, stock:stock}) FROM DATAFILE {
          PATH: "file://${TEST_DIR}/dataset/external_source/multi_type/node/product.csv",
          FORMAT: "csv",
      ...
      """
    Then an Error should be raised: "[NP401]: Connector error: Failed to convert node/edge type: contains, error: Connector error: Column: 'src_id' not found in source schema"
    And drop the graph "#contains_error"
    When executing analytic query:
      """
      CREATE GRAPH TYPE IF NOT EXISTS wrong_pk_type AS {
        NODE TYPE user (LABEL user {id INT PRIMARY KEY, name STRING}),
        EDGE TYPE purchase (user)-[LABEL purchase {MULTIEDGE KEY()}]->(user)
      }
      """
    Then the execution should be successful
    When executing analytic query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #multi_type_wrong_pk TYPED wrong_pk_type PARTITION BY DEFAULT
      """
    Then the execution should be successful
    When executing analytic query:
      """
      USE #multi_type_wrong_pk IMPORT INTO GRAPH
      {
        NODE (v@user{id:user_id, name:name}) FROM DATAFILE {
          PATH: "file://${TEST_DIR}/dataset/external_source/multi_type/node/user.csv",
          FORMAT: "csv",
          delimiter: ","
        }
      ...
      """
    Then an Error should be raised: "CSV conversion error to int64: invalid value"
    And drop the graph "#multi_type_wrong_pk"
    And drop the graph type "wrong_pk_type"
    And drop the graph type "order_type"
