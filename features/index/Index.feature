...
Feature: Index
  ...
  Scenario: query with index
    ...
    Then the result should be, in any order:
      | id | name    | age |
      | 3  | "Jerry" | 22  |
    # If the index in the hint doesn't exist, the index hint will be ignored.
    When executing query:
      """
      /*+ SET_VAR(optimizer_rules = "get_node_to_node_scan=on") */
      USE index_query_graph
    ...
    Then the result should be, in any order:
      | id | name    | age |
      | 3  | "Jerry" | 22  |
    # If the index in the hint is not appliable, the index hint will be ignored.
    When executing query:
      """
      USE index_query_graph
