  ...
  Scenario: Load distributed temporary graph from external data source
    ...
    When executing graph analytic query:
      ...
      USE #graph_projection_gen {
          ...
          VALUE place  SumAgg<INT> = 0
          VALUE likes  SumAgg<INT> = 0
          VALUE livein SumAgg<INT> = 0
          VALUE item   SumAgg<INT> = 0
      	MATCH(v:person)
      	PER NODE (v){
      	  SET @person += 1
      	}
      	...
      	RETURN @person, @place, @likes, @livein, @item
      }
      """
    Then the result should be, in any order:
      | @person | @place | @likes | @livein | @item |
      | 0       | 0      | 0      | 0       | 0     |
    When executing graph analytic query:
      """
      /*+ SET_VAR(query_concurrency = 4) */
