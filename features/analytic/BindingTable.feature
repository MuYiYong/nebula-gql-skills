  ...
  Scenario: Error cases
    When executing analytic query:
      ...
      USE #analytic_ldbc {
        ...
        NEXT
        FOR r2 IN t1
        RETURN r2.id AS t1_id, r2.name AS t1_name
      }
      """
    Then an Error should be raised: "Cannot mix distributed tables with non-distributed tables or graphs in the same query. Distributed tables: [t], Non-distributed variables: [t1]"
    When executing analytic query:
      """
      USE #analytic_ldbc {
        TABLE t TYPED TABLE {id INT, name STRING} PARTITION BY DEFAULT
      ...
      """
    Then an Error should be raised: "Procedure `show_functions` is not allowed to be executed on analytic worker"
