...
Feature: IndexOrderSatisfiesTopNRule
  ...
  Scenario: Query-to-note TopN with split outbound edge types
    ...
    When executing query:
      """
      CREATE INDEX IF NOT EXISTS idx_qcn_src_score_level ON EDGE QueryClickNote(src_id ASC, score ASC, level ASC)
      """
    Then the execution should be successful
    When executing query:
      """
      CREATE INDEX IF NOT EXISTS idx_qin_src_score_level ON EDGE QueryInfluencedByNote(src_id ASC, score ASC, level ASC)
      """
    Then the execution should be successful
    And index "idx_qcn_src_score_level" of "topn_idx_edge_graph" should be ready to use
    And index "idx_qin_src_score_level" of "topn_idx_edge_graph" should be ready to use
    When executing query:
      """
    ...
    When executing query:
      """
      /*+ SET_VAR(edge_index_prefixed_with_pk=true) */
      USE topn_idx_edge_graph
      MATCH (q)-[e:QUERY_CLICK_NOTE|QUERY_INFLUENCED_BY_NOTE]->(n)
      WHERE e.src_id = 1
      ORDER BY e.src_id, e.score, e.level
      LIMIT 2
    ...
    When executing query:
      ...
      USE topn_idx_edge_graph
      MATCH (q)-[e:QUERY_CLICK_NOTE|QUERY_INFLUENCED_BY_NOTE]->(n)
      WHERE e.src_id = 1 and e.level >= 2
      ORDER BY e.src_id, e.score, e.level
      LIMIT 2
      RETURN e.src_id AS srcId, e.score AS score, e.level AS level, n.id AS noteId
      """
    ...
    Then the result should be, in order:
      ...
      | 1     | 10    | 1     | 101    |
      | 1     | 15    | 1     | 101    |
    And drop the index "idx_qcn_src_score_level" of "topn_idx_edge_graph"
    And drop the index "idx_qin_src_score_level" of "topn_idx_edge_graph"
