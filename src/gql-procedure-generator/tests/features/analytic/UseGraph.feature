...
Feature: UseGraph
  ...
  Scenario: create distributed temp graph with qualified graph type path
    And drop the graph "#qualified_path_dist_graph"
    And drop the graph type "qualified_path_gt"
    When executing graph analytic query:
      """
      CREATE SCHEMA IF NOT EXISTS /test/qualified_path_schema
      """
    Then the execution should be successful
    When executing graph analytic query:
      """
      CREATE SCHEMA IF NOT EXISTS /test/default_schema
      """
    Then the execution should be successful
    When executing query:
      """
      SESSION SET SCHEMA /test/default_schema
      """
    Then the execution should be successful
    When executing graph analytic query:
      """
      CREATE GRAPH TYPE IF NOT EXISTS /test/qualified_path_schema/qualified_path_gt AS {
        NODE Person (LABEL Person {id INT64 PRIMARY KEY, name STRING})
      }
      """
    Then the execution should be successful
    And graph type "/test/qualified_path_schema/qualified_path_gt" should be ready to use
    When executing analytic query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #qualified_path_dist_graph TYPED /test/qualified_path_schema/qualified_path_gt PARTITION BY DEFAULT
      """
    Then the execution should be successful
    And drop the graph "#qualified_path_dist_graph"
    And drop the graph type "/test/qualified_path_schema/qualified_path_gt"
    When executing graph analytic query:
      """
      DROP SCHEMA IF EXISTS /test/qualified_path_schema
      """
    Then the execution should be successful
    When executing graph analytic query:
      """
      DROP SCHEMA IF EXISTS /test/default_schema
      """
    Then the execution should be successful
