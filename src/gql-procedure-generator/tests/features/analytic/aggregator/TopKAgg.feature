...
Feature: TopKAgg
  ...
  Scenario: basic
    ...
    When executing graph analytic query:
      """
      USE #dist_topk_agg_basic_test {
        VALUE topk TopKAgg<1, score INT DESC, id INT DESC>
        SET @topk += RECORD{id:5, score:14}
        SET @topk += RECORD{id:7, score:19}
        SET @topk += RECORD{id:8, score:19}
        SET @topk += RECORD{id:10, score:14}
