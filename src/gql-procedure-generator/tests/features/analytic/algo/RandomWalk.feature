...
Feature: RandomWalk
  ...
  @tt2
  Scenario: RandomWalk on graphd immutable temporary graph
    When executing query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #random_walk_ldbc AS COPY OF ldbc OPTIONS {immutable: true}
      """
    Then the execution should be successful
    When executing query:
      """
    ...
    Then the result should be, in any order:
      ...
      | LIST [2,3,2,3] |
    And drop the procedure "random_walk"
    And drop the graph "#random_walk_ldbc"
