...
Feature: PageRank
  ...
  Scenario: PageRank on a small graph
    ...
    When executing graph analytic query:
      """
      CREATE OR REPLACE PROCEDURE pagerank(node_label STRING, edge_label STRING, damping DOUBLE, max_iter INT) RETURNS (final_delta DOUBLE, id INT64, rank DOUBLE) AS {
        ...
        MATCH (s:node_label)-[e:edge_label]->(t:node_label)
        PER PATH {
          SET s.@out_edge += 1
        }
    ...
    When executing graph analytic query:
      """
      CREATE OR REPLACE PROCEDURE pagerank_on_type(node_type STRING, edge_type STRING, damping DOUBLE, max_iter INT) RETURNS (final_delta DOUBLE, id INT64, rank DOUBLE) AS {
        VALUE      cur_iter      INT64  = 1
        VALUE      add_constant  DOUBLE = 1.0 - 0.85
        VALUE      delta         SumAgg<DOUBLE> = 1.0
        VALUE      total_nodes   SumAgg<INT>    = 0
        ...
        MATCH (s@node_type)-[e@edge_type]->(t@node_type)
        PER PATH {
          SET s.@out_edge += 1
        }
    ...
    When executing query:
      ...
      USE #mutgraph_for_pagerank CALL pagerank("Person", "KNOWS", 0.85, 30) RETURN  id, rank
      """
    ...
    When executing query:
      """
      /*+ SET_VAR(query_concurrency = 4) */
      USE #mutgraph_for_pagerank CALL pagerank_on_type("Person", "KNOWS", 0.85, 30) RETURN id, rank
      """
    Then the result should be, in any order:
      | id | rank    |
      | 4  | 0.31818 |
    ...
    When executing analytic query:
      """
      SUBMIT USE #simple_graph_for_page_rank_dist_projection CALL pagerank("Person", "KNOWS", 0.85, 30) RETURN  id, rank
      """
    Then the result should be, in any order:
      | procedure_id | status      |
      | /.+/         | "SUBMITTED" |
    When executing graph analytic query:
      """
      USE #simple_graph_for_page_rank_dist_projection CALL pagerank("Person", "KNOWS", 0.85, 30) RETURN id, rank
      """
    Then the result should be, in any order:
      | id | rank    |
      | 4  | 0.31818 |
    ...
    When executing graph analytic query:
      """
      USE #simple_graph_for_page_rank_dist_projection CALL pagerank_on_type("Person", "KNOWS", 0.85, 30) RETURN id, rank
      """
    Then the result should be, in any order:
      | id | rank    |
      | 4  | 0.31818 |
    ...
    When executing graph analytic query:
      ...
      CALL pagerank("Person", "KNOWS", i, 30)
      RETURN rank, id, i
      """
    Then the result should be, in any order:
      | rank    | id | i    |
    ...
    When executing graph analytic query:
      """
      CALL pagerank("Person", "KNOWS", 0.85, 1) RETURN *
      """
    ...
    When executing query:
      """
      DROP PROCEDURE pagerank_on_type
      """
    Then the execution should be successful
    ...
    And drop the procedure "pagerank_on_type"
