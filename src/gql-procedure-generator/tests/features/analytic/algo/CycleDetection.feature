...
Feature: Cycle Detection
  ...
  Scenario: Cycle Detection
        ...
        WHILE @has_active = true AND iter < depth THEN {
          ...
          PER PATH {
            ...
            WHILE seq_idx < seq_count THEN {
              ...
              IF sequence[0] > t_id THEN {
                  SET seq_idx = seq_idx + 1
                  CONTINUE
              }

              IF t_id = sequence[0] THEN {
                IF is_min_of_list(t_id, sequence) THEN {
                    EXPORT sequence INTO cycles_table
                }
              } ELSEIF list_contains(sequence, t_id) = false THEN {
    ...
    When executing graph analytic query:
      """
      CREATE OR REPLACE PROCEDURE cycle_detection_batch (depth INT64, batch_num INT64) RETURNS (cycle LIST<INT64>) AS {
        VALUE frontier ACTIVE_SET
        VALUE has_active OrAgg = true
        ...
        NODE VALUE curr_list ListAgg<LIST<INT64>>
        NODE VALUE new_list ListAgg<LIST<INT64>>
        VALUE batch_idx INT64 = 0

        WHILE batch_idx < batch_num THEN {
            VALUE iter INT64 = 0
            MATCH (s) WHERE s.id % batch_num = batch_idx
            PER NODE (s) {
              SET s.@curr_list = [[s.id]]
            }
            FINALLY {
              ...
              PER PATH {
                ...
                WHILE seq_idx < seq_count THEN {
                  ...
                  IF t_id = sequence[0] THEN {
                    ...
                    SET t.@new_list += array_append(sequence,t_id)
                  }
    ...
    When executing graph analytic query:
      """
      USE #cycle_detection_g
      CALL cycle_detection_batch(10,3) RETURN *
      """
    Then the result should be, in any order:
      | cycle          |
      | LIST[3,4,5]    |
