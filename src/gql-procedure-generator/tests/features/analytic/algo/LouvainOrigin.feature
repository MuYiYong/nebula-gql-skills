...
Feature: Louvain
  ...
  Scenario: Louvain procedure
    When executing graph analytic query:
      """
      CREATE GRAPH TYPE IF NOT EXISTS algo_louvain_test_gt_origin {
    ...
    When executing graph query:
      """
      CREATE GRAPH IF NOT EXISTS algo_louvain_test_g_origin algo_louvain_test_gt_origin
      """
    Then the execution should be successful
    And graph "algo_louvain_test_g_origin" should be ready to use
    When executing graph query:
      """
      USE algo_louvain_test_g_origin
      FOR i IN range(0, 33)
    ...
    When executing graph query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #dist_algo_louvain_test_g_origin AS COPY OF algo_louvain_test_g_origin
      """
    Then the execution should be successful
    When executing analytic query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #dist_algo_louvain_test_g_origin TYPED algo_louvain_test_gt_origin PARTITION BY DEFAULT
      """
    Then the execution should be successful
    When executing analytic query:
      """
      USE #dist_algo_louvain_test_g_origin IMPORT INTO GRAPH
      {
        GRAPH FROM NEBULA{
          PATH:"nebula://root:NebulaGraph01@${TCK_GRAPH_ADDRESS}?graph=algo_louvain_test_g_origin&tls_enable=${TCK_GRAPH_TLS_ENABLED}&tls_ca=${TCK_GRAPH_TLS_CA_CERT}&tls_cert=${TCK_GRAPH_TLS_CERT}&tls_key=${TCK_GRAPH_TLS_KEY}&tls_peer_name=graph.server.vesoft.com"
        }
      }
      """
    Then the execution should be successful
    When executing graph analytic query:
      """
      CREATE PROCEDURE IF NOT EXISTS louvain_origin_origin(max_iter INT) RETURNS (max_id INT, min_id INT, community_size INT) AS {
    ...
    When executing graph analytic query:
      ...
      USE #dist_algo_louvain_test_g_origin CALL louvain_origin_origin(10) RETURN max_id, min_id, community_size as sz
      """
    ...
    When executing graph analytic query:
      """
      CREATE OR REPLACE PROCEDURE louvain_cross_neighbor_origin(max_iter INT) RETURNS (max_id INT, min_id INT, community_size INT) AS {
        VALUE      total_weight          SumAgg<DOUBLE> = 0
        NODE VALUE node_weight           SumAgg<DOUBLE> = 0               // total edge weight of each node
        NODE VALUE node_community_weight MapAgg<INT, SumAgg<DOUBLE>>      // total edge weight of each node for each community
    ...
    When executing graph analytic query:
      """
      /*+ SET_VAR(query_concurrency = 4) */
      USE #dist_algo_louvain_test_g_origin CALL louvain_cross_neighbor_origin(10) RETURN max_id, min_id, community_size as sz
      """
    Then the result should be, in any order:
      | max_id | min_id | sz |
      | 7      | 2      | 2  |
    ...
    When executing graph analytic query:
      """
      CREATE OR REPLACE PROCEDURE louvain_batch_origin(max_iter INT, num_of_batch INT) RETURNS (max_id INT, min_id INT, community_size INT) AS {
        VALUE      total_weight          SumAgg<DOUBLE> = 0
        NODE VALUE node_weight           SumAgg<DOUBLE> = 0               // total edge weight of each node
        NODE VALUE node_community_weight MapAgg<INT, SumAgg<DOUBLE>>      // total edge weight of each node for each community
        NODE VALUE community_weight      SumAgg<DOUBLE> = 0               // total edge weight of each community
    ...
    When executing graph analytic query:
      """
      /*+ SET_VAR(query_concurrency = 4) */
      USE #dist_algo_louvain_test_g_origin CALL louvain_batch_origin(10, 3) RETURN max_id, min_id, community_size as sz
      """
    Then the result should be, in any order:
      | max_id | min_id | sz |
      | 7      | 2      | 2  |
      ...
      | 29     | 23     | 5  |
      | 32     | 14     | 3  |
    And drop the graph "#dist_algo_louvain_test_g_origin"
    And drop the graph "algo_louvain_test_g_origin"
    And drop the graph type "algo_louvain_test_gt_origin"
    And drop the procedure "louvain_origin_origin"
    And drop the procedure "louvain_cross_neighbor_origin"
    And drop the procedure "louvain_batch_origin"

  Scenario: Louvain graph variable implementation
    When executing graph query:
      """
      CREATE GRAPH TYPE IF NOT EXISTS algo_louvain_full_test_gt_origin {
        NODE TYPE Person (LABEL Person {id INT PRIMARY KEY}),
        EDGE TYPE KNOWS (Person)~[LABEL KNOWS {weight DOUBLE}]~(Person)
      }
      """
    ...
    When executing graph query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #algo_louvain_test_g_origin algo_louvain_full_test_gt_origin
      """
    Then the execution should be successful
    And graph "#algo_louvain_test_g_origin" should be ready to use
    When executing graph query:
    ...
    When executing graph query:
      """
      CREATE GRAPH TYPE IF NOT EXISTS louvain_work_gt_origin AS {
        NODE TYPE community (
          LABEL community {
            id INT PRIMARY KEY,
            cid INT
    ...
    When executing graph query:
      """
      CREATE OR REPLACE PROCEDURE louvain_origin_import_graph(nodes TABLE, edges TABLE) AS {
        IMPORT INTO GRAPH {
    ...
    When executing graph query:
      """
      CREATE OR REPLACE PROCEDURE louvain_origin_outer(max_layer INT, max_iter INT) RETURNS (node_id INT, community_id INT) AS {
        GRAPH g TYPED louvain_work_gt_origin
        ...
        USE g
        CALL louvain_origin_import_graph(nodes, edges) FINISH

        SET nodes.clear()
        SET edges.clear()

        WHILE (i < max_layer) THEN {
          LOG_INFO("----------- louvain_outer_ori layer start, i = ", i)
          USE g
          CALL louvain_origin_inner(max_iter, nodes, edges) FINISH
    ...
    When executing graph query:
      """
      CREATE OR REPLACE PROCEDURE louvain_origin_inner(max_iter INT, nodes TABLE, edges TABLE) AS {
        VALUE      total_weight          SumAgg<DOUBLE> = 0
        ...
        LOG_INFO("louvain_inner_ori done, iter = ", iter)
    ...
    When executing graph query:
      ...
      /*+ SET_VAR(query_concurrency = 4) */
      USE #algo_louvain_test_g_origin
      CALL louvain_origin_outer(6, 6) RETURN count(DISTINCT community_id) AS community_cnt
      """
    Then the result should be, in any order:
      | community_cnt |
      | 3             |
    And drop the procedure "louvain_origin_import_graph"
    And drop the procedure "louvain_origin_outer"
    And drop the procedure "louvain_origin_inner"
    And drop the graph "#algo_louvain_test_g_origin"
    And drop the graph type "louvain_work_gt_origin"
    And drop the graph type "algo_louvain_full_test_gt_origin"
