  ...
  Scenario: DML operations using distributed table with nebula_exec
    ...
    When executing analytic query:
      ...
      USE #ldbc_analytic_distributed_test {
        ...
        PER PARTITION (data_part) OF new_person_data {
          FOR split IN table_split(data_part, CAST(4 as UINT))
    ...
    When executing analytic query:
      ...
      PER PARTITION (interest_data) OF new_interest_data {
      FOR split IN table_split(interest_data, CAST(2 as UINT))
    ...
    When executing analytic query:
      ...
      PER PARTITION (update_data_part) OF update_data {
        FOR split IN table_split(update_data_part, CAST(4 as UINT))
    ...
    When executing analytic query:
      ...
      PER PARTITION (edge_update_data_part) OF edge_update_data {
        FOR split IN table_split(edge_update_data_part, CAST(3 as UINT))
        CALL nebula_exec("nebula://root:NebulaGraph01@${TCK_GRAPH_ADDRESS}", fmt(fmt_edge_update, to_literal(split))) FINISH
      }
      }
      """
    ...
    When executing analytic query:
      ...
      PER PARTITION (delete_edge_data_part) OF delete_edge_data {
        FOR split IN table_split(delete_edge_data_part, CAST(3 as UINT))
        CALL nebula_exec("nebula://root:NebulaGraph01@${TCK_GRAPH_ADDRESS}", fmt(fmt_delete_edge, to_literal(split))) FINISH
      }
      }
      """
    ...
    When executing analytic query:
      ...
      PER PARTITION (detach_delete_data_part) OF detach_delete_data {
        FOR split IN table_split(detach_delete_data_part, CAST(7 as UINT))
        CALL nebula_exec("nebula://root:NebulaGraph01@${TCK_GRAPH_ADDRESS}", fmt(fmt_detach_delete, to_literal(split))) FINISH
      }
      }
      """
