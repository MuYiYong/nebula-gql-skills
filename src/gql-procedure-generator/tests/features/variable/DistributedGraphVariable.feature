...
Feature: Distributed Graph Variable
  ...
  @non_tls
  Scenario: Distributed graph variable direct local use
    # Setup graph type and base graph
    When executing graph analytic query:
      """
      CREATE GRAPH TYPE IF NOT EXISTS dgv_local_type_001 {
        NODE TYPE Person (LABEL Person {id INT PRIMARY KEY, name STRING})
      }
      """
    Then the execution should be successful
    When executing graph query:
      """
      CREATE GRAPH IF NOT EXISTS dgv_local_base_001 TYPED dgv_local_type_001
      """
    Then the execution should be successful
    And graph "dgv_local_base_001" should be ready to use
    When executing graph query:
      """
      USE dgv_local_base_001
      FOR i IN range(0, 2)
      INSERT (a@Person{id:i, name: 'person'})
      """
    Then the execution should be successful
    # Test direct use on distributed graph variable after import
    When executing analytic query:
      """
      GRAPH dgv_local_001 TYPED dgv_local_type_001 PARTITION BY DEFAULT
      TABLE result_table TYPED TABLE {id INT64, name STRING}
    ...
    Then the result should be, in any order:
      | id | name |
    And drop the graph "dgv_local_base_001"
    And drop the graph type "dgv_local_type_001"
