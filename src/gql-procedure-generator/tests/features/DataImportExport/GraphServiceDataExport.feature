...
Feature: Graph Service Data Export
  ...
  # Fix https://github.com/vesoft-inc/nebula-ng/issues/10613
  Scenario: Export with duplicate expression aliases should not crash
    Then remove the temporary directory "${TCK_SERVER_DIR}/output/output_csv_file_tck/export_dup_alias"
    When executing query:
      """
      FILE f1 {a INT64, b INT64, c INT64, d INT64} = DATAFILE {PATH:"file:///${TCK_SERVER_DIR}/output/output_csv_file_tck/export_dup_alias", FORMAT:'CSV'}
      USE ldbc MATCH (v:Person{id:1})
      EXPORT v.id, v.id, 1, 1 INTO f1
      """
    Then the execution should be successful
    And the query stats should be, in any order:
      | name                   | value |
