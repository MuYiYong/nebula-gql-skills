...
Feature: BindingTableImport
  ...
  Scenario: Import from local binding table into distributed temporary graph
    And drop the graph type "import_local_binding_table_dist_gt"
    And drop the graph "#import_local_binding_table_dist_g"
    When executing analytic query:
      """
      CREATE GRAPH TYPE IF NOT EXISTS import_local_binding_table_dist_gt AS {
        NODE Person (
          LABEL Person {
            id INT PRIMARY KEY,
            date_col STRING
    ...
    When executing analytic query:
      """
      CREATE TEMPORARY GRAPH IF NOT EXISTS #import_local_binding_table_dist_g TYPED import_local_binding_table_dist_gt PARTITION BY DEFAULT
      """
    Then the execution should be successful
    When executing analytic query:
      """
      /*+ set_var(query_concurrency=10) */
      USE #import_local_binding_table_dist_g {
        TABLE nodes TYPED TABLE {id INT, date_col STRING} =
          {id: 1, date_col: "xixi"},
          {id: 3, date_col: "xixi"},
          {id: 4, date_col: "xixi"},
    ...
    When executing analytic query:
      """
      USE #import_local_binding_table_dist_g SHOW STATS
      """
    Then the result should contain:
      | entry_name   | element_type | total_num |
      | "Node Total" | "Node"       | 4         |
