...
This is the complete public function allowlist for `gql-procedure-generator`. Every listed function may be generated when its documented signature, target environment, input types, and prerequisites match. Absence from feature files or high-frequency examples is not a reason to reject a listed function.
...
These are receiver methods, not global functions. Generate them only on the documented aggregator type.
