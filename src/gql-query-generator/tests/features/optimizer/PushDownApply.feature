...
Feature: PushDownApply
  ...
  Scenario: Correlated ExtendInto rewrite shapes
    # Case 1: local -> corr dst.
    # Force expansion from local country, then avoid introducing corr dst by cross join.
    # Rewrite to Extend(All) plus a join with DelimScan(dst).
    When executing query:
      """
      EXPLAIN DECORRELATED format="verbose"
      USE ldbc {
      MATCH (person:Person{id:1})-[:KNOWS]->(friend:Person)
      RETURN DISTINCT friend
      NEXT
      MATCH /*+ NO_REORDER */ (country:Country)<-[:IS_LOCATED_IN]-(message:Message)-[:HAS_CREATOR]->(friend)
      RETURN friend.id AS friendId, count(*) AS cnt GROUP BY friendId
      }
      """
    Then the execution should be successful
    And the plan should not contain "Extend(Into)"
    And the plan should not contain "LogicalJoin(Cross)"
    # Case 2: corr var1 -> corr var2 with ECP off.
    # Keep native ExtendInto because corr dst is already in the child input.
    When executing query:
      """
      EXPLAIN DECORRELATED format="verbose"
      /*+ SET_VAR(optimizer_rules = "enhanced_column_pruner=off") */
      USE ldbc {
      MATCH (person:Person{id:1})-[:KNOWS]->(friend:Person)
      RETURN person, friend
      NEXT
      MATCH (person)-[:KNOWS]->(friend)
      RETURN person.id AS personId, friend.id AS friendId
      }
      """
    Then the execution should be successful
    And the plan should contain "Extend(Into)"
    And the plan should not contain "LogicalJoin(Cross)"
    # Case 3: corr var1 -> corr var2 with ECP on.
    # Current rewrite still factorizes to Extend(All) plus DelimScan(dst) join.
    When executing query:
      """
      EXPLAIN DECORRELATED format="verbose"
