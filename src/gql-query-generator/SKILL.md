...
3. 先生成保持语义的最小基线查询。用户要求性能优化、查询形状可能放大中间结果或提供了 schema/索引/PROFILE 时，再按 [performance.md](references/performance.md) 生成候选；不要把依赖数据分布的猜测直接写进基线。
4. 按下方路由只加载与请求相关的参考。函数先在完整函数目录中按名称精确检索；其它语法先在完整语法目录中按关键字精确检索，再按需读取专题参考。
5. 生成后逐项执行 [validation.md](references/validation.md) 的检查；文档目录内的能力不因缺少 feature 或常用示例而拒绝，只有文档限制或已确认的实现冲突才触发降级。性能结论还必须标明是结构推断、静态计划、隔离 PROFILE 还是业务数据验证。
...
- 性能优化、索引与锚点、CBO/Hint、TopN/LIMIT、相关子查询、行数控制和 PROFILE 判读：读取 [performance.md](references/performance.md)。
...
- 默认 `enable_reorder=false` 时，将已绑定或高选择性锚点放在路径左侧；只有用户明确要求或计划证据支持时才启用 CBO/添加 `NO_REORDER`，不要默认生成执行 Hint。
- 在不改变 `OPTIONAL MATCH`、NULL 保留、相关作用域或路径集合语义时，单变量属性过滤优先写在对应 pattern filler；跨变量、路径变量、子查询和结果级条件保留在其正确作用域。不要把语法位置本身宣称为必然的性能提升，optimizer 也会按依赖下推部分外层谓词。
...
- 同一排序分页片段保持 `ORDER BY -> OFFSET/SKIP -> LIMIT`。5.3.0 允许该片段位于 `RETURN` 前或作为 `RETURN` 尾部；默认采用一种一致风格，不要误判另一种合法形式，也不要生成 `RETURN ... LIMIT ... ORDER BY ...`。
- 参数化查询使用 `$name`，不要把参数名写成字符串字面量。
- 用户要求“只给查询”时只输出代码块；否则最多补充关键假设和必要说明。
- 用户明确要求“确保性能”时，除查询外至少给出影响计划的 schema/索引前提和证据状态；没有当前数据与 PROFILE 时写“待验证”，不要声称“最优”或“足够高效”。

## Evidence Policy

将 v5.3.0 用户文档作为完整公开能力清单，将随包携带的 `.feature` 正反例和代码作为实现边界验证。目录内能力不需要 feature 或高频示例二次授权；目录外能力不得由 feature/代码单独授权。文档与已确认实现边界冲突时，按目标环境说明限制并回退，不用内部别名绕过。性能验证先证明结果与错误边界等价，再比较同图、同参数、同 Hint 条件下的 PROFILE 与交替多次运行；空图、单次耗时和隔离小图都不能外推为业务 SLA。
