...
description: '根据自然语言生成、改写或补全 GQL Procedure/UDP。Use when translating user intent into server-side programming, graph algorithms, traversal logic, match_compute_statement, user-defined procedures, named procedures, inline procedures, CREATE PROCEDURE, ALTER PROCEDURE, DROP PROCEDURE, CALL, OPTIONAL CALL, control flow, match compute, variable manipulation, logging, SHOW PROCEDURES, or SHOW CREATE PROCEDURE in this repo.'
argument-hint: '要生成的 GQL Procedure 需求、过程逻辑、参数、返回结果或自然语言描述'
user-invocable: true
---
...
6. 如果是多跳算法，先改写成 `WHILE` + 单轮 `match_compute_statement`。
