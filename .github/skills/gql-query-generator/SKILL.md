...
description: '根据自然语言生成、改写或补全 GQL 查询。Use when translating user intent into GQL query syntax, DQL, MATCH, FILTER, RETURN, WHERE, YIELD, ORDER BY, GROUP BY, LIMIT, OFFSET, CALL, graph patterns, expressions, variables, statement blocks, nearest-neighbor queries, parameterized queries, error-code rewrite, or query examples in this repo. Also use when converting, migrating, or translating Cypher (openCypher) queries or nGQL (NebulaGraph 2.x/3.x) queries into GQL. Do not use for graph algorithms or traversal procedures that should use match_compute_statement.'
argument-hint: '要生成的 GQL 查询需求、筛选条件、返回字段、自然语言描述，或需要转换的 Cypher/nGQL 语句'
user-invocable: true
---
...
- 详细参考材料在 `./references/` 目录下，按需加载。
- 规则同时参考 `nebula-ng` 实现与 ISO/IEC 39075:2024；冲突时以代码实现为准。
- 当 feature 测试行为与文档冲突时，以 feature 为准。

## References（按需加载）
- [expressions.md](./references/expressions.md) — 表达式、谓词、运算符
- [patterns.md](./references/patterns.md) — 图模式、路径、量词、过滤放置
- [functions.md](./references/functions.md) — 函数家族、lambda、legacy 迁移
- [error-codes.md](./references/error-codes.md) — 错误码改写映射与决策树
- [nearest-neighbor.md](./references/nearest-neighbor.md) — KNN/ANN 查询模板
- [migration.md](./references/migration.md) — Cypher/nGQL → GQL 语法映射与迁移决策
...
- 详细模式规则见 [patterns.md](./references/patterns.md)。
...
2. **若输入是 Cypher 或 nGQL 语句**，加载 [migration.md](./references/migration.md)，识别语言特征并按映射表逐条改写。
3. 确定查询主干：`MATCH` / `CALL procedure(...)` / `CALL { ... }`。
4. 抽取图模式、过滤条件、返回列、聚合、排序分页。
5. Schema 不足时继续生成草稿，不确定部分用占位符（`<Tag>`、`<prop>`）。
6. 若涉及错误码，加载 [error-codes.md](./references/error-codes.md) 按优先级修复。
7. 若涉及向量近邻，加载 [nearest-neighbor.md](./references/nearest-neighbor.md) 套模板。
...
- 不保留 nGQL/Cypher 的 `exists(v.prop)`——必须改为 `v.prop IS NOT NULL`（GQL 无 `PROPERTY_EXISTS` 函数）。
