...
改写要点：`exists(v.tag.prop)` → `v.prop IS NOT NULL`（GQL 无 `PROPERTY_EXISTS` 函数）；`v.tag.prop` → `v.prop`。
...
改写要点：Cypher `exists(n.prop)` → GQL `n.prop IS NOT NULL`（GQL 无 `PROPERTY_EXISTS` 函数）。
...
- 不要保留 nGQL/Cypher `exists(v.tag.prop)` 或 `exists(v.prop)`；改为 `v.prop IS NOT NULL`（GQL 无 `PROPERTY_EXISTS` 函数）。
