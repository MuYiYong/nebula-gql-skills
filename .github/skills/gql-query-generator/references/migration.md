...
| `keys(n)` | 无直接等价 — 逐属性返回 | |
| `exists(n.prop)` | `n.prop IS NOT NULL` | 属性存在检查（GQL 无 `PROPERTY_EXISTS` 函数） |
...
| nGQL `exists(v.tag.prop)` 或 Cypher `exists(n.prop)` 直接保留 | 改为 `v.prop IS NOT NULL`（GQL 无 `PROPERTY_EXISTS` 函数） |
